...
void sha3_224_inc_init(sha3_224incctx *state) {
    state->ctx = malloc(PQC_SHAKEINCCTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
...
void sha3_224_inc_ctx_clone(sha3_224incctx *dest, const sha3_224incctx *src) {
    dest->ctx = malloc(PQC_SHAKEINCCTX_BYTES);
    if (dest->ctx == NULL) {
        exit(111);
    }
...
void sha3_224_inc_ctx_release(sha3_224incctx *state) {
    free(state->ctx);
}

void sha3_224_inc_absorb(sha3_224incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, SHA3_224_RATE, input, inlen);
}

void sha3_224_inc_finalize(uint8_t *output, sha3_224incctx *state) {
    uint8_t t[SHA3_224_RATE];
    keccak_inc_finalize(state->ctx, SHA3_224_RATE, 0x06);

    keccak_squeezeblocks(t, 1, state->ctx, SHA3_224_RATE);

    sha3_224_inc_ctx_release(state);
 ...
 * Name:        sha3_224
 *
 * Description: SHA3-224 with non-incremental API
 *
 * Arguments:   - uint8_t *output:      pointer to output
 *              - const uint8_t *input: pointer to input
 *              - size_t inlen:   length of input in bytes
...
void sha3_224(uint8_t *output, const uint8_t *input, size_t inlen) {
    ...
    /* Absorb input */
    keccak_absorb(s, SHA3_224_RATE, input, inlen, 0x06);

    /* Squeeze output */
    keccak_squeezeblocks(t, 1, s, SHA3_224_RATE);
